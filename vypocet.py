...
def goal_mid(post1,post2): # najdi střed branky
    return [post1[0] + (post2[0]-post1[0])/2, post1[2] + (post2[2]-post1[2])/2]

def ball_corr(ball,me):
    ball_dist_vec = [ball[0]-me[0],ball[2]-me[1]]
...
def kick_pos(ball,post1,post2): # najdi místo výkopu
    mid = goal_mid(post1, post2)
    new_ball = ball_corr(ball, [0,0]) #[0,0] je moje pozice, počítám s tím že jsem v začátku
...
def dist_angle(me, ball, kick): # místo výkopu do úhlu(úhel od toho, když se koukáš přímo na míč) a vzdálenosti od aktuální pozice
    ...
    ang_corr = math.tan(new_ball[0]/new_ball[1]) #uhel od ted k mistu
    print(ang)
    print(ang_corr)
    return kick_dist, ang - ang_corr, -(ang + 1.57)